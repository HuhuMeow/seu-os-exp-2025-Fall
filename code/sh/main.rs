...
fn main() {
    ...
    while ok{
        ...
        match cmd{
            ...
            _ => {
                if let Some(full_path) = find_executable(cmd, &path_dirs){
                    let status = Command::new(cmd).args(argstr.split_whitespace()).status();
                    match status{
                        Ok(exit_code) => glob_exit_code = exit_code.code().unwrap_or(-1),
                        Err(e) => eprintln!("{}: execution error: {}", cmd, e),
                    }
                }else{
